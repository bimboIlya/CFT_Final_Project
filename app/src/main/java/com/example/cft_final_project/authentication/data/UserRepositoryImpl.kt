...
class UserRepositoryImpl(
    ...
    override suspend fun register(name: String, password: String) = withContext(dispatcherIO) {
    ...
    override suspend fun login(name: String, password: String) = withContext(dispatcherIO) {
        return@withContext try {
            val credentials = AuthParams(name, password)

            val tokenApi = authApiService.login(credentials)?.string()
            val token: String = tokenApi ?: throw MissingBaseDataException()
